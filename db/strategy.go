...
func QueryStrategies(tpls map[int]*model.Template) (map[int]*model.Strategy, error) {
	...
	sql := fmt.Sprintf(
		`select %s from strategy as s where (s.run_begin='' and s.run_end='') or 
		(s.run_begin < s.run_end and (s.run_begin <= '%s' and s.run_end >= '%s'))
		 or (s.run_begin >= s.run_end and (s.run_begin <= '%s' or s.run_end >= '%s'))`,
